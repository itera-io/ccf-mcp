...
- **Standalone VMs**: After `create-standalone-vm` (or other standalone VM mutations such as disk or flavor changes), run `commit-project` for that project so changes are provisioned, unless your organization auto-commits. Use `create-project` with **`standalone: true`** (non-Kubernetes project) when you only need VMs; `commit-project` on a Kubernetes project requires bastion, masters, and workers. Kubernetes-flavored projects can still run standalone VMs, but commit semantics follow that cluster layout.
